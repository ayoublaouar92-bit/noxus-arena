...
export default function GuestDebts() {
  ...
  async function topUpPlayer(player: Player) {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      setError("أدخل مبلغ شحن صحيح");
      return;
    }
    try {
      setBusy(true);
      setError("");
      const result = await api.topUpPlayer({ playerId: player.id, amount, note: "شحن المحفظة من صفحة الديون والمحافظ" });
      window.alert(`تم شحن المحفظة\nالمبلغ: ${money(result.amount)}\nالرصيد الجديد: ${money(result.walletBalance)}`);
      await load(true);
    } catch (e) {
      console.error(e);
  ...
  async function correctPlayerWallet(player: Player) {
    ...
    const raw = window.prompt(`الرصيد الحالي: ${money(current)}\n\nأدخل الرصيد الصحيح:`, current.toFixed(2));
    if (raw === null) return;
    const newBalance = Number(raw.replace(",", "."));
    if (!Number.isFinite(newBalance) || newBalance < 0) {
      setError("أدخل رصيدًا صحيحًا");
      return;
    }
    const reason = window.prompt("سبب التصحيح:", "تصحيح رصيد المحفظة") || "";
    if (!window.confirm(`تأكيد تصحيح محفظة ${player.name}؟\nمن ${money(current)} إلى ${money(newBalance)}`)) return;
    try {
      setBusy(true);
      setError("");
  ...
  async function payPlayerDebt(player: Player, full = false) {
    ...
    if (currentDebt <= 0) {
      setError("هذا اللاعب ليس عليه دين");
      return;
    }
    let amount = currentDebt;
    if (!full) {
      const raw = window.prompt(`الدين الحالي: ${money(currentDebt)}\n\nأدخل المبلغ الذي دفعه اللاعب:`);
      if (raw === null) return;
      amount = Number(raw.replace(",", "."));
    }
    if (!Number.isFinite(amount) || amount <= 0 || amount > currentDebt) {
      setError("مبلغ التسديد غير صحيح");
      return;
    }
    if (!window.confirm(`تأكيد تحصيل ${money(amount)} من ${player.name}؟\nالمتبقي: ${money(currentDebt - amount)}`)) return;
    try {
      setBusy(true);
      setError("");
      await api.payPlayerDebt({ playerId: player.id, amount, note: full ? "تسديد كامل لدين اللاعب" : "تسديد جزئي لدين اللاعب" });
      await load(true);
    } catch (e) {
      console.error(e);
      setError("تعذر تسديد دين اللاعب");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div dir="rtl" className="space-y-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-semibold">الديون والمحافظ / Finance</h1>
          <p className="mt-2 text-sm text-white/45">محافظ اللاعبين + ديون اللاعبين + ديون الضيوف</p>
        </div>
        ...
        <article className="rounded-xl border border-white/[0.08] bg-[#0c101d]">
          ...
          <div className="p-5">
            <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
              <div className="rounded-xl border border-white/[0.08] bg-[#090d18] p-4"><p className="text-xs text-white/35">ديون الضيوف المفتوحة</p><p dir="ltr" className="mt-2 text-lg font-semibold text-amber-300">{money(totals.totalOpen)}</p></div>
              <div className="rounded-xl border border-white/[0.08] bg-[#090d18] p-4"><p className="text-xs text-white/35">ديون اللاعبين</p><p dir="ltr" className="mt-2 text-lg font-semibold text-rose-300">{money(totals.playerDebtOpen)}</p></div>
              <div className="rounded-xl border border-white/[0.08] bg-[#090d18] p-4"><p className="text-xs text-white/35">محافظ اللاعبين</p><p dir="ltr" className="mt-2 text-lg font-semibold text-sky-300">{money(totals.playerWalletTotal)}</p></div>
              <div className="rounded-xl border border-white/[0.08] bg-[#090d18] p-4"><p className="text-xs text-white/35">محصل من الضيوف</p><p dir="ltr" className="mt-2 text-lg font-semibold text-emerald-300">{money(totals.totalCollected)}</p></div>
            </div>

            <div className="mt-4 rounded-xl border border-sky-400/15 bg-[#090d18] p-4">
              <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div><h2 className="font-semibold">محافظ وديون اللاعبين</h2><p className="mt-1 text-xs text-white/35">بحث، شحن محفظة، تصحيح رصيد، وتسديد الدين جزئيًا أو كاملًا</p></div>
                <div className="flex h-10 items-center gap-2 rounded-lg border border-white/10 bg-[#080b16] px-3 lg:w-80"><Search size={15} className="text-white/25" /><input value={playerQuery} onChange={(e) => setPlayerQuery(e.target.value)} placeholder="بحث عن لاعب..." className="min-w-0 flex-1 bg-transparent text-sm outline-none" /></div>
              </div>
              {filteredPlayers.length === 0 ? <div className="rounded-lg bg-white/[0.03] p-6 text-center text-sm text-white/35">لا يوجد لاعبون مطابقون أو لا توجد ديون لاعبين مفتوحة</div> : (
                <div className="grid gap-3 lg:grid-cols-2">{filteredPlayers.slice(0, 80).map((player) => <div key={player.id} className="rounded-lg border border-white/[0.08] bg-[#080b16] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{player.name}</p><p className="mt-1 text-xs text-white/35">@{player.username} · {player.phone || "—"}</p></div><div className="text-left" dir="ltr"><p className="text-xs text-sky-300">Wallet {money(player.walletBalance)}</p><p className="text-xs font-semibold text-rose-300">Debt {money(player.debtBalance)}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" disabled={busy} onClick={() => void topUpPlayer(player)} className="h-9 rounded-lg bg-emerald-500/15 text-xs text-emerald-300 disabled:opacity-40">شحن المحفظة</button><button type="button" disabled={busy} onClick={() => void correctPlayerWallet(player)} className="h-9 rounded-lg bg-sky-500/15 text-xs text-sky-300 disabled:opacity-40">تصحيح المحفظة</button><button type="button" disabled={busy || Number(player.debtBalance || 0) <= 0} onClick={() => void payPlayerDebt(player, false)} className="h-9 rounded-lg bg-amber-500/15 text-xs text-amber-300 disabled:opacity-30">تسديد جزئي</button><button type="button" disabled={busy || Number(player.debtBalance || 0) <= 0} onClick={() => void payPlayerDebt(player, true)} className="h-9 rounded-lg bg-rose-500/15 text-xs text-rose-300 disabled:opacity-30">تسديد كامل</button></div></div>)}</div>
              )}
            </div>
            <h2 className="mt-6 mb-3 font-semibold">ديون الضيوف</h2>
