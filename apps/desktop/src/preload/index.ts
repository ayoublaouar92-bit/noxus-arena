...
const api = {
  ...
  createStaffUser: (data: { name: string; role: "Admin" | "Staff"; pin: string }) =>
  ...
  addPlayer: (player: { name: string; username: string; phone?: string; initialDeposit?: number; image?: string }) =>
    ipcRenderer.invoke("finance:add-player", player),
  deletePlayer: (playerId: number) => ipcRenderer.invoke("finance:delete-player", playerId),
  ...
  addCategory: (data: { name: string; color?: string; sortOrder?: number; active?: boolean }) =>
    ipcRenderer.invoke("store:add-category", data),
  updateCategory: (data: { categoryId: number; name?: string; color?: string; sortOrder?: number; active?: boolean }) =>
    ipcRenderer.invoke("store:update-category", data),
  deleteCategory: (categoryId: number) => ipcRenderer.invoke("store:delete-category", categoryId),
  ...
  addExpense: (data: { title: string; category: string; amount: number; note?: string; spentAt?: string }) =>
    ipcRenderer.invoke("billing:add-expense", data),
  deleteExpense: (expenseId: number) => ipcRenderer.invoke("billing:delete-expense", expenseId),

  // Reports
  getReport: (payload: { range: "today" | "week" | "month" | "custom"; start?: string; end?: string }) =>
