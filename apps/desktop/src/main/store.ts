...
function ensureStoreTables(db: any) {
  ...
  const saleColumns = db.prepare("PRAGMA table_info(sales)").all() as Array<{
    name: string;
  }>;
  const saleColumnNames = new Set(saleColumns.map((column) => column.name));
  if (!saleColumnNames.has("shiftId"))
    db.exec(`ALTER TABLE sales ADD COLUMN shiftId INTEGER;`);
  if (!saleColumnNames.has("staffUserId"))
    db.exec(`ALTER TABLE sales ADD COLUMN staffUserId INTEGER;`);
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_sales_staff ON sales(staffUserId);
    CREATE INDEX IF NOT EXISTS idx_sale_items_sale ON sale_items(saleId);
  `);

  const existingOther = db
    .prepare(
      `SELECT id FROM product_categories WHERE LOWER(name) = LOWER('Other') LIMIT 1`,
    )
...
export function registerStoreHandlers(db: any) {
  ...
  registerHandler("store:add-category", (_event, data: CreateCategoryData) => {
    ...
    const result = db
      .prepare(
        `INSERT INTO product_categories (name, color, sortOrder, active) VALUES (?, ?, ?, ?)`,
      )
  ...
  registerHandler(
    ...
    (_event, data: UpdateCategoryData) => {
      ...
      db.prepare(
        `UPDATE product_categories SET name = ?, color = ?, sortOrder = ?, active = ? WHERE id = ?`,
      ).run(
        next.name,
        next.color || null,
        next.sortOrder,
  ...
  registerHandler("store:delete-category", (_event, categoryId: number) => {
    ...
    const row = db
      .prepare(`SELECT name FROM product_categories WHERE id = ?`)
    ...
    if (Number(used.total) > 0) {
      const tx = db.transaction(() => {
        db.prepare(`UPDATE product_categories SET active = 0 WHERE id = ?`).run(
          categoryId,
        );
        db.prepare(
          `UPDATE products SET categoryId = ? WHERE categoryId = ?`,
        ).run(otherId, categoryId);
      });
      tx();
  ...
  registerHandler("store:add-product", (_event, data: CreateProductData) => {
    ...
    const tx = db.transaction(() => {
      ...
      if (initialStock > 0) {
        db.prepare(
          `INSERT INTO inventory_movements (productId, quantity, reason, note) VALUES (?, ?, 'PURCHASE', 'Initial stock')`,
  ...
  registerHandler(
    "store:get-product-movements",
    (_event, productId: number) => {
      return db
        .prepare(
          `SELECT * FROM inventory_movements WHERE productId = ? ORDER BY id DESC LIMIT 200`,
        )
        .all(productId);
    },
  );
  ...
  registerHandler("store:move-stock", (_event, data: StockMoveData) => {
    ...
    const tx = db.transaction(() => {
      ...
      db.prepare(
        `INSERT INTO inventory_movements (productId, quantity, reason, note) VALUES (?, ?, ?, ?)`,
  ...
  registerHandler("store:create-sale", (_event, data: CreateSaleData) => {
    const staffUserId = requireStaff(db, "STORE_CREATE_SALE");
    ...
    const tx = db.transaction(() => {
      ...
      const saleResult = db
        .prepare(
          `INSERT INTO sales
           (customerName, paymentType, playerId, subtotal, total, walletPaid, debtAdded, cashPaid, status, note, shiftId, staffUserId)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Completed', ?, ?, ?)`,
        )
        .run(
          customerName,
