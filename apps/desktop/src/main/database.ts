...
const hadWalletBalance =
  playerColumnNames.has(
    "walletBalance"
  );

const hadDebtBalance =
  playerColumnNames.has(
    "debtBalance"
  );
...
for (
  const migration
  of sessionMigrations
) {
  if (
    !sessionColumnNames.has(
      migration.name
    )
  ) {
    db.exec(
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS
    idx_sessions_player
  ON sessions(playerId);

  CREATE INDEX IF NOT EXISTS
    idx_wallet_transactions_player
  ON wallet_transactions(playerId);

  CREATE INDEX IF NOT EXISTS
    idx_guest_debts_status
  ON guest_debts(status);

  CREATE INDEX IF NOT EXISTS
    idx_guest_debts_phone
