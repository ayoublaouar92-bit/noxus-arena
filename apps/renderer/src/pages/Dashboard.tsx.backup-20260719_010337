...
export default function Dashboard() {
  ...
  async function loadDashboard(showLoading = false) {
    try {
      ...
      setError("تعذر تحميل لوحة التحكم / Dashboard failed to load");
    } finally {
      setLoading(false);
    }
  }
  ...
  function getPrice(startTime: string, hourlyPrice: string) {
    const minutes = getMinutes(startTime);

    return ((minutes / 60) * Number(hourlyPrice || 0)).toFixed(2);
  }

  function formatStartTime(startTime: string) {
    return new Date(startTime).toLocaleTimeString("ar-DZ", {
      hour: "2-digit",
      minute: "2-digit",
    });
  }
  ...
  const busyDevices = useMemo(
    () => devices.filter((device) => device.status === "Busy"),
    [devices],
  );
  ...
  const occupancyRate =
    devices.length === 0
      ? 0
      : Math.round((busyDevices.length / devices.length) * 100);

  const estimatedRevenue = sessions.reduce(
    (total, session) =>
      total + Number(getPrice(session.startTime, session.hourlyPrice)),
    0,
  );
  ...
  const averageDuration =
    sessions.length === 0
      ...
      : Math.round(
          sessions.reduce(
            (total, session) => total + getMinutes(session.startTime),
            0,
          ) / sessions.length,
        );
  ...
  return (
    <div dir="rtl" className="noxus-page noxus-dashboard space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <article
              key={stat.english}
              className="noxus-stat-card group rounded-xl border border-white/[0.08] bg-[#0c101d] p-5 transition hover:border-violet-400/20"
            >
      ...
      <section className="grid gap-6 2xl:grid-cols-[minmax(0,1.45fr)_minmax(340px,0.55fr)]">
        <article className="noxus-hero relative min-h-[330px] overflow-hidden rounded-xl border border-violet-400/15 bg-[#0b0f1c]">
          ...
          <div className="relative z-10 flex h-full min-h-[330px] flex-col justify-between p-6 lg:p-8">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h2 className="mt-5 max-w-lg text-3xl font-semibold leading-tight lg:text-4xl">
                  القاعة تعمل بكفاءة
                  <span dir="ltr" className="mt-2 block text-violet-300">
                    Arena Operations Online
                  </span>
                </h2>

                <p className="mt-4 max-w-lg text-sm leading-6 text-white/40">
                  تابع إشغال الأجهزة والجلسات النشطة والإيرادات من مكان واحد.
                </p>
              </div>
              ...
              <div className="hidden h-28 w-28 items-center justify-center rounded-full border border-violet-400/20 bg-violet-500/[0.06] lg:flex">
                <div className="text-center">
                  ...
                  <p className="mt-1 text-[10px] text-white/35">OCCUPANCY</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="mb-3 flex items-center justify-between text-xs">
                <span className="text-white/40">نسبة إشغال القاعة</span>

                <span dir="ltr" className="text-violet-300">
                  {busyDevices.length} / {devices.length} devices
                </span>
              </div>
        ...
        <article className="rounded-xl border border-white/[0.08] bg-[#0c101d]">
          <div className="flex items-center justify-between border-b border-white/[0.08] p-5">
            <div>
              <h2 className="font-semibold">حالة الأجهزة</h2>
          ...
          <div className="space-y-2 p-4">
            {devices.length === 0 ? (
              <div className="flex min-h-56 flex-col items-center justify-center text-center">
                <Monitor size={28} className="mb-3 text-white/20" />

                <p className="text-sm text-white/35">لا توجد أجهزة</p>
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p dir="ltr" className="mt-1 text-[10px] text-white/30">
                        {device.type} · {device.price || "0"} DA/h
                      </p>
                    </div>
          ...
          <div className="grid grid-cols-2 gap-3 border-t border-white/[0.08] p-4">
            <div className="rounded-lg bg-emerald-500/[0.06] p-3">
              <p className="text-xs text-white/35">متاحة</p>
            ...
            <div className="rounded-lg bg-rose-500/[0.06] p-3">
              <p className="text-xs text-white/35">مشغولة</p>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        <article className="rounded-xl border border-white/[0.08] bg-[#0c101d]">
          <div className="flex items-center justify-between border-b border-white/[0.08] p-5">
            <div>
              <h2 className="font-semibold">الجلسات المباشرة</h2>
          ...
          <div className="p-4">
            {sessions.length === 0 ? (
              <div className="flex min-h-44 flex-col items-center justify-center text-center">
                <Clock3 size={28} className="mb-3 text-white/20" />

                <p className="text-sm text-white/35">لا توجد جلسات نشطة الآن</p>
              </div>
            ) : (
                ...
                {sessions.slice(0, 3).map((session) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span dir="ltr" className="text-[10px] text-white/25">
                        {formatStartTime(session.startTime)}
                      </span>
                    </div>

                    <h3 dir="ltr" className="mt-4 truncate font-semibold">
                      {session.deviceName}
                    </h3>
                    ...
                    <div className="mt-4 flex items-end justify-between">
                      <div>
                        ...
                        <p dir="ltr" className="mt-1 text-sm text-sky-300">
                      ...
                      <div className="text-left">
                        ...
                        <p dir="ltr" className="mt-1 text-sm text-emerald-300">
                          {getPrice(session.startTime, session.hourlyPrice)} DA
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <article className="rounded-xl border border-white/[0.08] bg-[#0c101d]">
          <div className="border-b border-white/[0.08] p-5">
            <h2 className="font-semibold">وصول سريع</h2>
