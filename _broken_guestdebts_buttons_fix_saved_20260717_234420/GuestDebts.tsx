...
export default function GuestDebts() {
  ...
  async function correctPlayerWallet(player: Player) {
    ...
    if (
      !window.confirm(
        `Confirm wallet correction for ${player.name}?\nFrom ${money(current)} to ${money(newBalance)}`,
      )
    ) {
      return;
    }
  ...
  async function payPlayerDebt(player: Player, full = false) {
    ...
    if (currentDebt <= 0) {
      window.alert("This player has no debt");
      return;
    }
    ...
    if (!full) {
      const raw = window.prompt(
        `Current debt: ${money(currentDebt)}\n\nEnter paid amount:`,
      );
      if (raw === null) return;
    ...
    if (
      !window.confirm(
        `Confirm collecting ${money(amount)} from ${player.name}?\nRemaining: ${money(currentDebt - amount)}`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      console.error(e);
      window.alert("Could not pay player debt");
    } finally {
      setBusy(false);
    }
  }
