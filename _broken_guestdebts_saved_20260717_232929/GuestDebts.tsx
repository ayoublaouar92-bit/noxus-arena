...
export default function GuestDebts() {
  ...
  async function load(showLoading = false) {
    try {
      ...
      console.error(e);
      setError("تعذر تحميل Guest debts");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function topUpPlayer(player: Player) {
    ...
    try {
      setBusy(true);
      setError("");
      const result = await api.topUpPlayer({ playerId: player.id, amount, note: "Top up wallet من صفحة الديون والمحافظ" });
      window.alert(`تم Top up wallet\nالمبلغ: ${money(result.amount)}\nالرصيد الجديد: ${money(result.walletBalance)}`);
      await load(true);
    } catch (e) {
      console.error(e);
      setError("تعذر Top up wallet");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function correctPlayerWallet(player: Player) {
    ...
    if (!Number.isFinite(newBalance) || newBalance < 0) {
      setError("Enter a valid wallet balance");
      return;
    }
    const reason = window.prompt("Correction reason:", "Wallet balance correction") || "";
  ...
  async function payPlayerDebt(player: Player, full = false) {
    ...
    if (!full) {
      const raw = window.prompt(`Current debt: ${money(currentDebt)}\n\nEnter paid amount:`);
      if (raw === null) return;
      amount = Number(raw.replace(",", "."));
    }
    if (!Number.isFinite(amount) || amount <= 0 || amount > currentDebt) {
      setError("Invalid debt payment amount");
      return;
    }
    ...
    try {
      setBusy(true);
      setError("");
      await api.payPlayerDebt({ playerId: player.id, amount, note: full ? "Full debt لدين اللاعب" : "Partial debt لدين اللاعب" });
      await load(true);
    } catch (e) {
      console.error(e);
  ...
  async function addManualDebt(event: FormEvent) {
    ...
    if (!canAddManual) {
      window.alert("Add manual guest debt: Admin فقط");
    ...
    try {
      ...
      else setError("تعذر Add manual guest debt");
    } finally {
      setBusy(false);
    }
  }
        ...
        <article className="rounded-xl border border-white/[0.08] bg-[#0c101d]">
          ...
          <div className="p-5">
            <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
              <div className="rounded-xl border border-white/[0.08] bg-[#090d18] p-4"><p className="text-xs text-white/35">Guest debts المفتوحة</p><p dir="ltr" className="mt-2 text-lg font-semibold text-amber-300">{money(totals.totalOpen)}</p></div>
            ...
            <div className="mt-4 rounded-xl border border-sky-400/15 bg-[#090d18] p-4">
              <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div><h2 className="font-semibold">Player wallets & debts</h2><p className="mt-1 text-xs text-white/35">Search players, top up wallets, correct balances, and collect debts</p></div>
              ...
              {filteredPlayers.length === 0 ? <div className="rounded-lg bg-white/[0.03] p-6 text-center text-sm text-white/35">No matching players or no open player debts</div> : (
                <div className="grid gap-3 lg:grid-cols-2">{filteredPlayers.slice(0, 80).map((player) => <div key={player.id} className="rounded-lg border border-white/[0.08] bg-[#080b16] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{player.name}</p><p className="mt-1 text-xs text-white/35">@{player.username} · {player.phone || "—"}</p></div><div className="text-left" dir="ltr"><p className="text-xs text-sky-300">Wallet {money(player.walletBalance)}</p><p className="text-xs font-semibold text-rose-300">Debt {money(player.debtBalance)}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2">
                          ...
                          onClick={() => {
                            ...
                            void (async () => {
                              try {
                                setBusy(true);
                                setError("");
                                await api.topUpPlayer({ playerId: player.id, amount, note: "Wallet top-up" });
                                await load(true);
                              } catch (e) {
                                console.error(e);
                                window.alert("Could not top up wallet");
                              } finally {
                                setBusy(false);
                              }
                            })();
                          ...
                          onClick={() => {
                            ...
                            void (async () => {
                              try {
                                setBusy(true);
                                setError("");
                                await api.setPlayerWalletBalance({ playerId: player.id, newBalance, reason: "Wallet correction" });
                                await load(true);
                              } catch (e) {
                                console.error(e);
                                window.alert("Could not correct wallet");
                              } finally {
                                setBusy(false);
                              }
                            })();
                        ...
                        <button
                          type="button"
                          disabled={busy || Number(player.debtBalance || 0) <= 0}
                          onClick={() => {
                            const currentDebt = Math.max(0, Number(player.debtBalance || 0));
                            const raw = window.prompt(`Current debt: ${money(currentDebt)}\n\nEnter paid amount:`);
                            if (raw === null) return;
                            ...
                            void (async () => {
                              try {
                                setBusy(true);
                                setError("");
                                await api.payPlayerDebt({ playerId: player.id, amount, note: "Partial player debt payment" });
                                await load(true);
                              } catch (e) {
                                console.error(e);
                                window.alert("Could not pay debt");
                              } finally {
                                setBusy(false);
                              }
                            })();
        ...
        <aside className="space-y-6">
          {canAddManual ? (
            <article className="rounded-xl border border-violet-400/15 bg-[#0c101d]">
              <div className="border-b border-white/[0.08] p-5">
                <h2 className="font-semibold">Add manual guest debt</h2>
            ...
            <article className="rounded-xl border border-amber-400/15 bg-[#0c101d]">
              <div className="p-5 text-sm text-amber-200">
                Add manual guest debt Admin فقط.
              </div>
            </article>
          )}
        </aside>
