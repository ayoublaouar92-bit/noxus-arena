...
function ensureTables(db: any) {
  ...
  const count = db
    .prepare(`SELECT COUNT(*) AS total FROM staff_users`)
    .get() as { total: number };
...
export function requireStaff(db: any, action: string) {
  ...
  const staff = db
    ...
    .get(currentStaffUserId) as
    { id: number; role: Role; active: number } | undefined;
...
export function registerStaffHandlers(db: any) {
  ...
  registerHandler("staff:login", (_event, data: LoginData) => {
      ...
      ? (db
          .prepare(
            `SELECT * FROM staff_users
             WHERE id = ? AND pinHash = ? AND active = 1
             LIMIT 1`,
          )
          .get(userId, sha256(pin)) as StaffUserRow | undefined)
      : (db
          .prepare(
            `SELECT * FROM staff_users
             WHERE pinHash = ? AND active = 1
             LIMIT 1`,
          )
    ...
    if (currentSessionId) {
      db.prepare(
        `UPDATE staff_sessions SET logoutAt = ? WHERE id = ? AND logoutAt IS NULL`,
      ).run(nowIso(), currentSessionId);
    }
  ...
  registerHandler("staff:list-users", () => {
    ...
    return db
      .prepare(
        `SELECT id, name, role, active, createdAt FROM staff_users ORDER BY role DESC, id ASC`,
      )
      .all();
  });
  ...
  registerHandler("staff:create-user", (_event, data: CreateUserData) => {
    ...
    const result = db
      .prepare(
        `INSERT INTO staff_users (name, role, pinHash, active) VALUES (?, ?, ?, 1)`,
      )
  ...
  // Admin only
  registerHandler(
    "staff:set-user-active",
    (_event, payload: { userId: number; active: boolean }) => {
      requireAdmin(db, "STAFF_SET_ACTIVE");

      db.prepare(`UPDATE staff_users SET active = ? WHERE id = ?`).run(
        payload.active ? 1 : 0,
        payload.userId,
      );
