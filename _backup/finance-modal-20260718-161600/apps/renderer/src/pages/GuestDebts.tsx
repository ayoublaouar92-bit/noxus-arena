...
export default function GuestDebts() {
  ...
  async function topUpPlayer(player: Player) {
    ...
    try {
      setBusy(true);
      setError("");
      const result = await api.topUpPlayer({
        playerId: player.id,
        amount,
        note: "Wallet top-up from finance page",
      });
      window.alert(
      ...
      console.error(e);
      if (handleUnauthorized(e)) return;
      setError(financeError(e, "تعذر شحن المحفظة"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function correctPlayerWallet(player: Player) {
    ...
    const raw = window.prompt(
      `Current wallet: ${money(current)}

Enter correct balance:`,
      current.toFixed(2),
    );
    if (raw === null) return;
    const newBalance = parseMoneyInput(raw);
    if (!Number.isFinite(newBalance) || newBalance < 0) {
      setError("Enter a valid balance");
      return;
    }
    const reason =
      window.prompt("Correction reason:", "Wallet correction") || "";
    if (
      !window.confirm(`Confirm wallet correction for ${player.name}?
From ${money(current)} to ${money(newBalance)}`)
    )
      return;
    try {
      setBusy(true);
      ...
      console.error(e);
      if (handleUnauthorized(e)) return;
      setError(financeError(e, "تعذر تصحيح المحفظة"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function payPlayerDebt(player: Player, full = false) {
    ...
    if (!full) {
      const raw = window.prompt(
        `دين ${player.name}: ${money(currentDebt)}\n\nأدخل المبلغ المدفوع:`,
      );
      if (raw === null) return;
...
Remaining: ${money(currentDebt - amount)}`)
    ...
    try {
      ...
      await api.payPlayerDebt({
        playerId: player.id,
        amount,
        note: full ? "Full player debt payment" : "Partial player debt payment",
      });
      await load(true);
    } catch (e: any) {
      console.error(e);
                    ...
                    >
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <button
                          type="button"
                          disabled={busy}
                          onClick={() => void topUpPlayer(player)}
                        ...
                        <button
                          type="button"
                          disabled={busy}
                          onClick={() => void correctPlayerWallet(player)}
                        ...
                        <button
                          ...
                          onClick={() => void payPlayerDebt(player, false)}
                        ...
                        <button
                          ...
                          onClick={() => void payPlayerDebt(player, true)}
