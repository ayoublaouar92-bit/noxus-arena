...
export function requireOpenShiftForCurrentStaff(db: any): ActiveShiftContext {
  const currentStaffUserId = getCurrentStaffUserId();
  if (!currentStaffUserId) throw new Error("UNAUTHORIZED");

  const shift = db
    .prepare(
      `SELECT id, staffUserId, openedAt
       FROM cash_shifts
       WHERE status = 'Open'
       ORDER BY id DESC
       LIMIT 1`,
    )
    .get() as ActiveShiftContext | undefined;

  if (!shift) throw new Error("SHIFT_REQUIRED");
  if (Number(shift.staffUserId) !== Number(currentStaffUserId)) {
...
export function registerBillingHandlers(db: any) {
  ...
  const expenseColumnNames = new Set(
    expenseColumns.map((column) => column.name),
  );
  if (!expenseColumnNames.has("shiftId"))
    db.exec(`ALTER TABLE expenses ADD COLUMN shiftId INTEGER;`);
  if (!expenseColumnNames.has("staffUserId"))
    db.exec(`ALTER TABLE expenses ADD COLUMN staffUserId INTEGER;`);
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_expenses_staff ON expenses(staffUserId);
  `);
  ...
  const calculateShiftTotals = (
    ...
    const rangeParams = closedAt ? [openedAt, closedAt] : [openedAt];
    ...
    const sumForRange = (
      ...
      const row = db
        ...
        .get(shiftId, ...rangeParams) as { total: number };
    ...
    const playerDebtCash = sumForRange(
      "wallet_transactions",
      "amount",
      "createdAt",
      "type = 'PLAYER_DEBT_PAYMENT' AND amount > 0",
    );
    const expenses = sumForRange("expenses", "amount", "spentAt", "amount > 0");

    const linkedDebtPayments = db
      .prepare(
        `SELECT COALESCE(SUM(amount), 0) AS total
         FROM guest_debt_payments
         WHERE shiftId = ?`,
      )
      .get(shiftId) as { total: number };

    let guestDebtCash = getNumber(linkedDebtPayments?.total);
    if (guestDebtCash === 0) {
      ...
      const debtAuditRows = db
        ...
        .all(...rangeParams) as Array<{ details?: string | null }>;

      guestDebtCash = debtAuditRows.reduce((total, row) => {
        try {
    ...
    const cashCollected =
      sessionCash + storeCash + walletTopUps + playerDebtCash + guestDebtCash;
  ...
  registerHandler(
    ...
    (_event, query: BillingTransactionsQuery | undefined) => {
      if (!getCurrentStaffUserId()) throw new Error("UNAUTHORIZED");
      ...
      if (range === "today") {
        ...
        startDate = query?.start ? new Date(query.start) : new Date(now);
        endDate = query?.end ? new Date(query.end) : new Date(now);
        startDate.setHours(0, 0, 0, 0);
        endDate.setHours(23, 59, 59, 999);
      }
      ...
      const sessions = db
        .prepare(
          `SELECT
             ...
             sessions.endTime AS occurredAt,
             devices.name AS deviceName,
             staff_users.name AS staffName
           FROM sessions
           LEFT JOIN devices ON devices.id = sessions.deviceId
           LEFT JOIN staff_users ON staff_users.id = sessions.staffUserId
           WHERE sessions.status = 'Finished'
             AND sessions.endTime IS NOT NULL
             AND datetime(sessions.endTime) >= datetime(?)
             AND datetime(sessions.endTime) <= datetime(?)
      ...
      for (const row of sessions) {
        const cashPaid = getNumber(row.cashPaid);
        const walletPaid = getNumber(row.walletPaid);
        const debtAdded = getNumber(row.debtAdded);
        transactions.push({
          id: `session-${row.id}`,
          sourceId: row.id,
          type: row.sessionType === "round" ? "Round" : "Session",
          typeLabel: row.sessionType === "round" ? "جولة لعب" : "جلسة لعب",
          customer: row.customerName || "Guest",
          details: row.deviceName || `Session #${row.id}`,
          payment:
            cashPaid > 0 ? "Cash" : debtAdded > 0 ? "Wallet + Debt" : "Wallet",
          revenue: getNumber(row.totalPrice),
          cashMovement: cashPaid,
          walletPaid,
          debtAdded,
          amount: getNumber(row.totalPrice),
          staffUserId: row.staffUserId,
          staffName: row.staffName || "غير محدد",
          shiftId: row.shiftId,
      ...
      const sales = db
        .prepare(
          `SELECT
             ...
             staff_users.name AS staffName
           FROM sales
           LEFT JOIN players ON players.id = sales.playerId
           LEFT JOIN staff_users ON staff_users.id = sales.staffUserId
      ...
      for (const row of sales) {
        ...
        transactions.push({
          ...
          sourceId: row.id,
          type: "Store",
          typeLabel: "مبيعات المتجر",
          customer: row.playerName || row.customerName || "Walk-in",
          details: row.note || `Sale #${row.id}`,
          payment:
            cashPaid > 0 ? "Cash" : debtAdded > 0 ? "Wallet + Debt" : "Wallet",
          revenue: getNumber(row.total),
          cashMovement: cashPaid,
          walletPaid,
          debtAdded,
      ...
      const topUps = db
        .prepare(
          `SELECT
             ...
             staff_users.name AS staffName
           FROM wallet_transactions
           INNER JOIN players ON players.id = wallet_transactions.playerId
           LEFT JOIN staff_users ON staff_users.id = wallet_transactions.staffUserId
           WHERE wallet_transactions.type = 'TOP_UP'
             AND datetime(wallet_transactions.createdAt) >= datetime(?)
             AND datetime(wallet_transactions.createdAt) <= datetime(?)
      ...
      for (const row of topUps) {
        transactions.push({
          ...
          type: "WalletTopUp",
          typeLabel: "شحن محفظة",
          customer: row.playerName,
          details: row.note || "Wallet top-up",
          payment: "Cash",
          revenue: 0,
          cashMovement: getNumber(row.amount),
          walletPaid: 0,
          debtAdded: 0,
      ...
      const playerDebtPayments = db
        .prepare(
          `SELECT
           ...
           INNER JOIN players ON players.id = wallet_transactions.playerId
           LEFT JOIN staff_users ON staff_users.id = wallet_transactions.staffUserId
           WHERE wallet_transactions.type = 'PLAYER_DEBT_PAYMENT'
             AND datetime(wallet_transactions.createdAt) >= datetime(?)
             AND datetime(wallet_transactions.createdAt) <= datetime(?)
           ORDER BY wallet_transactions.id DESC
           LIMIT 1000`,
      ...
      for (const row of playerDebtPayments) {
        transactions.push({
          id: `player-debt-payment-${row.id}`,
          sourceId: row.id,
          type: "PlayerDebtPayment",
          typeLabel: "تسديد دين لاعب",
          customer: row.playerName,
          details: row.note || "Player debt payment",
          payment: "Cash",
          revenue: 0,
          cashMovement: getNumber(row.amount),
          walletPaid: 0,
      ...
      const debtPayments = db
        .prepare(
          `SELECT
             guest_debt_payments.*,
             guest_debts.guestName,
             staff_users.name AS staffName
           FROM guest_debt_payments
           INNER JOIN guest_debts ON guest_debts.id = guest_debt_payments.guestDebtId
           LEFT JOIN staff_users ON staff_users.id = guest_debt_payments.staffUserId
           WHERE datetime(guest_debt_payments.paidAt) >= datetime(?)
             AND datetime(guest_debt_payments.paidAt) <= datetime(?)
           ORDER BY guest_debt_payments.id DESC
           LIMIT 1000`,
        )
        .all(startIso, endIso);
      ...
      for (const row of debtPayments) {
        transactions.push({
          ...
          type: "DebtPayment",
          typeLabel: "تسديد دين",
          customer: row.guestName || "Guest",
          details: row.note || `Debt #${row.guestDebtId}`,
          payment: "Cash",
          revenue: 0,
          cashMovement: getNumber(row.amount),
          walletPaid: 0,
      ...
      const expenseRows = db
        .prepare(
          `SELECT expenses.*, staff_users.name AS staffName
           FROM expenses
           LEFT JOIN staff_users ON staff_users.id = expenses.staffUserId
           WHERE datetime(expenses.spentAt) >= datetime(?)
             AND datetime(expenses.spentAt) <= datetime(?)
      ...
      for (const row of expenseRows) {
        transactions.push({
          ...
          type: "Expense",
          typeLabel: "مصروف",
          customer: row.category || "Other",
          details: row.note || row.title,
          payment: "Cash out",
          revenue: 0,
          cashMovement: -getNumber(row.amount),
          walletPaid: 0,
      ...
      const filtered = transactions
        .filter((item) =>
          query?.staffUserId
            ? Number(item.staffUserId) === Number(query.staffUserId)
            : true,
        )
        .filter((item) =>
          query?.shiftId
            ? Number(item.shiftId) === Number(query.shiftId)
            : true,
        )
        .filter((item) =>
          query?.type && query.type !== "All" ? item.type === query.type : true,
        )
        .filter((item) =>
          query?.payment && query.payment !== "All"
            ? item.payment === query.payment
            : true,
        )
        .filter((item) => {
          if (!normalizedSearch) return true;
          return `${item.customer} ${item.details} ${item.staffName}`
            .toLowerCase()
      ...
      const totals = filtered.reduce(
        (result, item) => {
          result.revenue += getNumber(item.revenue);
          result.cashMovement += getNumber(item.cashMovement);
          result.cashIn += Math.max(0, getNumber(item.cashMovement));
          result.walletPaid += getNumber(item.walletPaid);
          result.debtAdded += getNumber(item.debtAdded);
          if (item.type === "Expense")
            result.expenses += getNumber(item.amount);
          if (item.type === "Session" || item.type === "Round")
            result.sessionRevenue += getNumber(item.revenue);
          if (item.type === "Store")
            result.storeRevenue += getNumber(item.revenue);
          return result;
        },
        {
          revenue: 0,
      ...
      return {
        ...
        totals,
        transactions: filtered.slice(0, 1000),
        staff,
      };
    },
  );
      ...
      .prepare(
        `
          ...
          LEFT JOIN staff_users
            ON staff_users.id = expenses.staffUserId
          ORDER BY datetime(expenses.spentAt) DESC, expenses.id DESC
        `,
      )
      .all();
  });

  // Any logged-in cashier can record an expense in their own open shift.
  registerHandler("billing:add-expense", (_event, data: AddExpenseData) => {
    const staffUserId = requireStaff(db, "BILLING_ADD_EXPENSE");
      ...
      .prepare(
        `
          INSERT INTO expenses
            (title, category, amount, note, spentAt, shiftId, staffUserId)
          VALUES
            (?, ?, ?, ?, ?, ?, ?)
        `,
      )
