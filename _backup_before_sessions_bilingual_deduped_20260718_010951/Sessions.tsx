...
Waiting: ${result.queued.length}`,
  ...
  async function addWaiting(playerId: number) {
    try {
      ...
      console.error(actionError);
      setError("Player is already waiting or has an active session / Player / Player / اللاعب موجود في الانتظار أو لديه جلسة نشطة");
    } finally {
      setBusy(false);
    }
  }
  async function removeWaiting(id: number) {
    try {
      ...
      console.error(actionError);
      setError("Could not remove player from waiting list / تعذر حذف Player / Player / اللاعب من الانتظار");
    } finally {
      setBusy(false);
    }
  }
...
Started next: ${result.started.length}`,
  ...
  async function startSingle(event: FormEvent) {
    ...
    if (!singleDevice) return setError("Select device / Select device / Select device / اختر الجهاز");
    if (singleCustomerType === "player" && !singlePlayer)
      return setError("Select player / اختر Player / Player / اللاعب");
    if (singleCustomerType === "guest" && !singleGuestName.trim())
      return setError("Enter guest name / أدخل Guest name / Guest name / اسم الضيف");
    ...
    try {
      ...
      console.error(actionError);
      setError("Could not start session / تعذر Start session / Start session / بدء الجلسة");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <article className="rounded-xl border border-amber-400/20 bg-[#0c101d]">
          <div className="border-b border-white/[0.08] p-5">
            ...
            <p className="mt-1 text-xs text-white/35">
              Search players and set price per player / ابحث وحدد Price / Price / السعر
            </p>
          </div>
          <div className="p-5">
            <div className="mb-3 flex h-11 items-center gap-2 rounded-lg border border-white/10 bg-[#080b16] px-3">
              ...
              <input
                ...
                placeholder="Search player name or username / ابحث باسم Player / Player / اللاعب"
        ...
        <aside className="rounded-xl border border-sky-400/20 bg-[#0c101d]">
          <div className="border-b border-white/[0.08] p-5">
            ...
            <p className="mt-1 text-xs text-white/35">
              New players are added at the bottom / يضاف Player / Player / اللاعب أسفل القائمة
            </p>
          </div>
          <div className="p-5">
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_350px]">
        <article className="rounded-xl border border-white/[0.08] bg-[#0c101d]">
          <div className="border-b border-white/[0.08] p-5">
            <h2 className="font-semibold">Active sessions / Active sessions / Active sessions / الجلسات النشطة</h2>
          </div>
            ...
            {sessions.map((session) => (
              ...
              >
                ...
                <div className="my-3 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-white/35">{session.playerId ? "Player / Player / Player / اللاعب" : "Guest"}</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-white/35">Duration / Duration / Duration / المدة</span>
                    <span>{minutes(session.startTime)} min</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-white/35">Price / Price / Price / السعر</span>
                ...
                {!session.roundGroupId &&
                  (session.playerId ? (
                    ...
                    >
                      <Banknote size={14} className="inline" /> Cash only / كاش فقط
                    </button>
                  ))}
              </article>
            ))}
        ...
        >
          <h2 className="mb-4 flex items-center gap-2 font-semibold">
            <UserRound size={18} /> Single session / Single session / Single session / جلسة فردية
          </h2>
          ...
          >
            <option value="">Select device / Select device / Select device / اختر الجهاز</option>
          ...
          {singleCustomerType === "player" ? (
            ...
            >
              <option value="">Select player / اختر Player / Player / اللاعب</option>
              {players.map((p) => (
            ...
            <div className="mt-3 space-y-3">
              <input
                ...
                placeholder="Guest name / Guest name / Guest name / اسم الضيف"
              />
              <input
                dir="ltr"
                className={fieldClass}
          ...
          {singleKind === "round" && (
            <input
              ...
              placeholder="Price / Price / Price / السعر DA"
            />
          )}
          <button
            disabled={busy}
